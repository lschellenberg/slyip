...
func SendTransaction(tx *types.Transaction, client *ethclient.Client, limit time.Duration, timeout time.Duration) bool {
	...
	if mined {
		...
		fmt.Printf("%s submitted but not mined\n", tx.Hash().Hex())
	}
...
func CreateTransactionOpts(client *ethclient.Client, tk *cryptox.Wallet) *bind.TransactOpts {
	//fetch the last use nonce of account
	nonce, err := client.PendingNonceAt(context.Background(), tk.Public)
	if err != nil {
		panic(err)
	}
	...
	auth, err := bind.NewKeyedTransactorWithChainID(tk.Private, chainID)
	if err != nil {
		panic(err)
	}
